...
- **Status:** draft (gate for Track A·6 — kept longest; the ONLY GHL-writable module)
- **Target home:** Own core (config KV) with GHL write-through while kept
- **Current state in repo:** `handlers/customvalues.go` (`GET /api/cv` fan-out, `POST /api/cv/bulk` parallel PUT, concurrency 6), `routes/custom-values/+page.svelte`. Core shipped; inner tabs + NPL scanner deferred (Phase 2c+).
...
7. **New Patient Link scanner** — scan CVs for the patient-booking link pattern _(deferred; needs definition — see Open questions)_.
...
## Open questions
- **New Patient Link scanner:** exact definition — what link pattern, which CV keys, what's the output/action?

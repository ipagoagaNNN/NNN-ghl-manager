...
func main() {
	...
	mux.HandleFunc("GET /api/cv/npl-scan", handlers.NPLScan(vault)) // New Patient Link audit (M3 D-3.1)
	mux.HandleFunc("POST /api/cv/bulk", handlers.BulkUpdateCV(vault))
	mux.HandleFunc("GET /api/cv/{locationId}/{cvId}", handlers.GetCustomValue(vault))       // single custom value (v2021-04-15)
	mux.HandleFunc("DELETE /api/cv/{locationId}/{cvId}", handlers.DeleteCustomValue(vault)) // delete custom value (v2021-04-15)
	mux.HandleFunc("GET /api/customfields/{locationId}", handlers.ListCustomFields(vault))  // custom-field schema (v2021-04-15)
	mux.HandleFunc("GET /api/objects/{locationId}", handlers.ListObjects(vault))            // object schema (v2023-02-21)
